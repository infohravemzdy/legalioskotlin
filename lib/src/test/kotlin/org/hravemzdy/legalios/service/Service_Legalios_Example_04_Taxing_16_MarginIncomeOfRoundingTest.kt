...
class Service_Legalios_Example_04_Taxing_16_MarginIncomeOfRoundingTest  : Spek({
    val testList = listOf(
    ...
    logTestIntExamples("04_Taxing_16_MarginIncomeOfRounding.txt", testList)
            ...
            tx.tests.forEach { tt ->
                context("period ${tt.title}") {
                    ...
                    it("GetProps should return value = ${tt.expected}") {
                        assertEquals(tt.expected, props?.marginIncomeOfRounding)
                    }
                }
            }
        }
