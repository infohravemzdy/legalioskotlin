...
class Service_Legalios_Example_01_Health_03_LimMonthlyStateTest  : Spek({
    val testList = listOf(
    ...
    logTestIntExamples("01_Health_03_LimMonthlyState.txt", testList)
            ...
            tx.tests.forEach { tt ->
                context("period ${tt.title}") {
                    ...
                    it("GetProps should return value = ${tt.expected}") {
                        assertEquals(tt.expected, props?.limMonthlyState)
                    }
                }
            }
        }
