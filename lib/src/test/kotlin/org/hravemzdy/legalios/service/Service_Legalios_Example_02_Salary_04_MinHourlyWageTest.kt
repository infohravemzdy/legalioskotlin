...
class Service_Legalios_Example_02_Salary_04_MinHourlyWageTest  : Spek({
    val testList = listOf(
    ...
    logTestIntExamples("02_Salary_04_MinHourlyWage.txt", testList)
            ...
            tx.tests.forEach { tt ->
                context("period ${tt.title}") {
                    ...
                    it("GetProps should return value = ${tt.expected}") {
                        assertEquals(tt.expected, props?.minHourlyWage)
                    }
                }
            }
        }
