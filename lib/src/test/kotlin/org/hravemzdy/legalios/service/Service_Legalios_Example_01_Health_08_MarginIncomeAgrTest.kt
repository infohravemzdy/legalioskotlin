...
class Service_Legalios_Example_01_Health_08_MarginIncomeAgrTest  : Spek({
    val testList = listOf(
    ...
    logTestIntExamples("01_Health_08_MarginIncomeAgr.txt", testList)
            ...
            tx.tests.forEach { tt ->
                context("period ${tt.title}") {
                    ...
                    it("GetProps should return value = ${tt.expected}") {
                        assertEquals(tt.expected, props?.marginIncomeAgr)
                    }
                }
            }
        }
